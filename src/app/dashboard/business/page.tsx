...
import { getHireRequestsByEmail, type HireRequestRow } from "@/lib/db";

const mockRequests: HireRequestRow[] = [
  { id: "hr_1", agentName: "DataAnalyst Pro", agentSlug: "data-analyst-pro", useCase: "Weekly revenue reports", budgetRange: "$100 - $250/mo", status: "accepted", createdAt: "2026-04-01" },
  { id: "hr_2", agentName: "CodeReviewer", agentSlug: "code-reviewer", useCase: "PR reviews for 3 repos", budgetRange: "< $100/mo", status: "pending", createdAt: "2026-04-02" },
  { id: "hr_3", agentName: "SupportAgent", agentSlug: "customer-support-ai", useCase: "Handle tier-1 tickets", budgetRange: "$250 - $500/mo", status: "pending", createdAt: "2026-04-03" },
  { id: "hr_4", agentName: "ContentCraft", agentSlug: "content-writer", useCase: "Blog post generation", budgetRange: "$100 - $250/mo", status: "declined", createdAt: "2026-03-28" },
];
...
export default async function BusinessDashboard() {
  ...
  // Load user's hire requests from Supabase, fall back to mock
  let requests = mockRequests;
  if (user?.email) {
    const dbRequests = await getHireRequestsByEmail(user.email);
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-ink/60">
            {user ? `Signed in as ${user.email}` : "Track your hire requests and agent deployments"}
          </p>
        </div>
        <Link
          href="/agents"
              ...
              {requests.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-ink/40">
                    No hire requests yet. <Link href="/agents" className="text-aegean hover:underline">Browse agents</Link> to get started.
                  </td>
                </tr>
              ) : (
                requests.map((req) => (
                  <tr key={req.id} className="border-b border-limestone/10 last:border-0">
                    ...
                    <td className="p-4">
                      <span className={`text-xs px-2 py-1 rounded-full ${statusColors[req.status] || "bg-gray-50 text-gray-700"}`}>
                        {req.status}
                      </span>
                    </td>
